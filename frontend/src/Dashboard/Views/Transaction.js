...
export default function Transaction(props) {
  ...
  const handleSportFilter = (event) => {
    ...
    console.log(sportsFilter, setTeams([]));
  };
          ...
          <CardContent>
            <Grid container spacing={4}>
              <Grid item xs={1}>
                ...
                <Select style={{ minWidth: 120 }} defaultValue={""} onChange={handleSportFilter}>
                  {sports.map((sport) => (
                  ...
                  {users.map((user) => {
                    return (
                      <ListItem key={user.id} dense button onClick={handleAthleteToggle(user.id)}>
                        <ListItemText primary={user.fullName} secondary={user.isActive} />
                        <ListItemIcon>
                          <Checkbox edge="start" checked={usersCheckbox.indexOf(user.id) !== -1} />
                        </ListItemIcon>
                      </ListItem>
                    );
                  })}
                  ...
                  {inventory.map((inventory) => {
                    return (
                      <ListItem key={inventory.id} dense button onClick={handleInventoryToggle(inventory.id)}>
                        <ListItemText primary={inventory.name} secondary={inventory.description} />
                        <ListItemIcon>
                          {/*<Checkbox
                                            edge="start"
